...
def test__create_product_team_bad_name(keys: list, name: str):
    ...
    with pytest.raises(ValidationError):
        org.create_product_team(name=name, keys=keys)
...
def test__create_product_team_from_org_no_id(
    ...
    result = org.create_product_team(keys=keys, name=name)
...
def test__create_product_team_from_org_id_raises_error(
    ...
    with pytest.raises(TypeError):
        org.create_product_team(id=id, name=name, keys=keys)
...
def test__create_product_team_with_incoming_params(
    ...
    result = org.create_product_team(**incoming_params.dict(exclude={"ods_code"}))
    event = result.events[0]
