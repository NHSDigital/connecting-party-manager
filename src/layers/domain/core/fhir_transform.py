...
def create_product_team_from_fhir_org_json(
    ...
    product_team = org.create_product_team(id=fhir_org.id, name=fhir_org.name)  # type: ignore
    return product_team


def create_fhir_model_from_product_team(
    product_team: ProductTeam, **kwargs
) -> cpm_model.Organization:
    return cpm_model.Organization(
