...
from domain.repository.repository.v3 import (
...
@pytest.fixture
def repository() -> "MyRepositoryV3":
    return MyRepositoryV3()


@pytest.mark.integration
def test_single_repository_write(repository: MyRepositoryV3):
    value = "123"
...
def test_writes_to_different_keys_split_over_batches_repository_write(
    repository: MyRepositoryV3, number_of_adds: int, number_of_batches: int
):
    my_item = MyModel(
        field="abc",
...
@pytest.mark.integration
def test_repository_raise_already_exists(repository: MyRepositoryV3):
...
@pytest.mark.integration
def test_repository_raise_already_exists_multiple_events(repository: MyRepositoryV3):
    my_item = MyModel(
        field="123",
        events=[
...
@pytest.mark.integration
def test_repository_add_and_delete_separate_transactions(repository: MyRepositoryV3):
    value = "123"
    my_item = MyModel(field=value, events=[MyEventAdd(field=value)])
    repository.write(my_item)
...
@pytest.mark.integration
def test_repository_write_bulk(repository: MyRepositoryV3):
