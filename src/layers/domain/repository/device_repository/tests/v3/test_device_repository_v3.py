...
@pytest.fixture
def another_device_with_same_key() -> DeviceV3:
    org = Root.create_ods_organisation(ods_code="AB123")
    product_team = org.create_product_team(name="Product Team")
    product = product_team.create_cpm_product(name="Product")
...
@pytest.mark.integration
def test__device_repository_already_exists(device, repository: DeviceRepositoryV3):
    repository.write(device)
...
def test__device_repository_key_already_exists_on_another_device(
    device, another_device_with_same_key, repository: DeviceRepositoryV3
):
    repository.write(device)
    with pytest.raises(AlreadyExistsError):
...
@pytest.mark.integration
def test__device_repository__update(device: DeviceV3, repository: DeviceRepositoryV3):
    repository.write(device)
...
def test__device_repository__delete(
    device_with_tag: DeviceV3, repository: DeviceRepositoryV3
):
...
@pytest.mark.integration
def test__device_repository__add_key(device: DeviceV3, repository: DeviceRepositoryV3):
    repository.write(device)
