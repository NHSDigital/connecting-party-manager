...
def nhs_accredited_system_to_cpm_devices(
    ...
    for (
        ...
        update_in_list_of_dict(
            obj=questionnaire_response_answers, key="nhs_as_client", value=[ods_code]
        )
        ...
        _device.add_key(
            key_type=DeviceKeyType.ACCREDITED_SYSTEM_ID, key_value=accredited_system_id
        )
        ...
        for tag_fields in SearchSDSDeviceQueryParams.allowed_field_combinations():
            tags = questionnaire_response_answers_to_device_tags(
                answers=questionnaire_response_answers, tag_fields=tag_fields
            )
            for tag in tags:
                _device.add_tag(**tag)
        _device.add_tag(unique_identifier=unique_identifier)
...
def nhs_mhs_to_cpm_device(nhs_mhs: NhsMhs) -> Device:
    ...
    for tag_fields in SearchSDSEndpointQueryParams.allowed_field_combinations():
        ...
        for tag in tags:
            device.add_tag(**tag)
    device.add_tag(unique_identifier=nhs_mhs.unique_identifier)
    return device
...
def translate_bulk(obj: dict[str, str]) -> list[Device]:
    ...
    return [device.state() for device in devices]


def translate_updates(
    obj: dict[str, str], repository: DeviceRepository
) -> ExportedEventsTypeDef:
    object_class = obj["object_class"].lower()
    if object_class == SdsDeletionRequest.OBJECT_CLASS:
...
def translate(obj: dict[str, str], repository: DeviceRepository):
    if obj.get("unique_identifier") in BAD_UNIQUE_IDENTIFIERS:
        return []

    try:
        return translate_bulk(obj=obj)
    except UnknownSdsModel:
        return translate_updates(obj=obj, repository=repository)
