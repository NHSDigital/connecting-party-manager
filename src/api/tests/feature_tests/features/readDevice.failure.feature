Feature: Read Device - failure scenarios
  ...
  Scenario: Read an unknown product teams Device
    ...
    And I have already made a "POST" request with "default" headers to "ProductTeam/${ note(product_team_id) }/Product/${ note(product_id) }/dev/Device" with body:
      ...
      | name | My Device |
    And I note the response field "$.id" as "device_id"
    When I make a "GET" request with "default" headers to "ProductTeam/F5H1R.f63ba1d2-99b3-4e7f-83b4-a98178f1e4fe/Product/${ note(product_id) }/dev/Device/${ note(device_id) }"
    Then I receive a status code "404" with body
      | path             | value                                                                             |
      | errors.0.code    | RESOURCE_NOT_FOUND                                                                |
  ...
  Scenario: Read an unknown Device
    ...
    And I have already made a "POST" request with "default" headers to "ProductTeam/${ note(product_team_id) }/Product/${ note(product_id) }/dev/Device" with body:
      ...
      | name | My Device |
    And I note the response field "$.id" as "device_id"
    When I make a "GET" request with "default" headers to "ProductTeam/${ note(product_team_id) }/Product/${ note(product_id) }/dev/Device/not-a-device"
    Then I receive a status code "404" with body
      | path             | value                                                                                                        |
      | errors.0.code    | RESOURCE_NOT_FOUND                                                                                           |
