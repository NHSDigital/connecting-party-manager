Feature: Create "Message Set" Device Reference Data - failure scenarios
  ...
  Scenario: Fail to create an "MHS Message Set" Device Reference Data, with invalid questionnaire response (unknown field)
    ...
    Then I receive a status code "400" with body
      | path             | value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
      | errors.0.code    | VALIDATION_ERROR                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
      | errors.0.message | Payload contains unexpected fields: {'unknown_field'}. Expected fields are: ['Contract Property Template Key', 'Interaction Type', 'MHS IN', 'MHS Is Authenticated', 'MHS SN', 'Reliability Configuration Ack Requested', 'Reliability Configuration Actor', 'Reliability Configuration Duplication Elimination', 'Reliability Configuration Persist Duration', 'Reliability Configuration Reply Mode', 'Reliability Configuration Retries', 'Reliability Configuration Retry Interval']. |
    And the response headers contain:
      | name           | value            |
      | Content-Type   | application/json |
      | Content-Length | 530              |
