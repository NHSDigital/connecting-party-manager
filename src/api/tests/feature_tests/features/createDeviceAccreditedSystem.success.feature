Feature: Create AS Device - success scenarios
  ...
  Scenario: Successfully create a AS Device with MHSMessageSet and ASAdditionalInteractions containing no questionnaire responses
    ...
    And I note the response field "$.keys.0.key_type" as "party_key_type"
    And I note the response field "$.keys.0.key_value" as "party_key"
    When I make a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/ProductEpr/${ note(product_id) }/dev/DeviceReferenceData/MhsMessageSet"
    And I note the response field "$.id" as "mhs_message_set_drd_id"
    When I make a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/ProductEpr/${ note(product_id) }/dev/DeviceReferenceData/AccreditedSystemsAdditionalInteractions"
    And I note the response field "$.id" as "as_message_set_drd_id"
    When I make a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/ProductEpr/${ note(product_id) }/dev/Device/AccreditedSystem" with body:
      | path                                               | value             |
      | questionnaire_responses.spine_as.0.ODS Code        | F5H1R             |
  ...
  Scenario: Successfully create multiple AS Devices
    ...
    And the response headers contain:
      ...
      | Content-Type   | application/json |
      | Content-Length | 1920             |
    When I make a "GET" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/ProductEpr/${ note(product_id) }/dev/Device/${ note(device_id_2) }"
    Then I receive a status code "200" with body
      | path                    | value                                         |
      | id                      | ${ note(device_id_2) }                        |
      | name                    | F5H1R-850000/200000100001 - Accredited System |
