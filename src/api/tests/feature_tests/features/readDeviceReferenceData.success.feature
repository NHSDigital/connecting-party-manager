Feature: Read Device Reference Data - success scenarios
  ...
  Scenario Outline: Successfully read a Device Reference Data
    ...
    When I make a "GET" request with "default" headers to "ProductTeam/<product_team_id>/Product/${ note(product_id) }/DeviceReferenceData/${ note(device_reference_data_id) }"
  ...
  Scenario Outline: Successfully read a Device Reference Data with a Product created for EPR
    ...
    And I have already made a "POST" request with "default" headers to "ProductTeam/${ note(product_team_id) }/Product/${ note(product_id) }/DeviceReferenceData" with body:
      ...
      | name | My Device Reference Data |
    And I note the response field "$.id" as "device_reference_data_id"
    When I make a "GET" request with "default" headers to "ProductTeam/<product_team_id>/Product/<product_id>/DeviceReferenceData/${ note(device_reference_data_id) }"
    Then I receive a status code "200" with body
      | path                    | value                               |
      | id                      | ${ note(device_reference_data_id) } |
      | name                    | My Device Reference Data            |
