Feature: Create Device - failure scenarios
  ...
  Scenario: Cannot create a Device with a Device that is missing fields (no name) and has extra param
    ...
    When I make a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/Product/${ note(product_id) }/dev/Device" with body:
      | path      | value     |
      | bad_field | My Device |
  ...
  Scenario: Cannot create a Device with a Product Team that does not exist
    When I make a "POST" request with "default" headers to "ProductTeamEpr/not-a-product-team/Product/not-a-product/dev/Device" with body:
      | path | value     |
      | name | My Device |
  ...
  Scenario: Cannot create a Device with a Product that does not exist
    Given I have already made a "POST" request with "default" headers to "ProductTeamEpr" with body:
      ...
      | ods_code | F5H1R                 |
    And I note the response field "$.id" as "product_team_id"
    When I make a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/Product/not-a-product/dev/Device" with body:
      | path | value     |
      | name | My Device |
    Then I receive a status code "404" with body
      | path             | value                                                                             |
  ...
  Scenario: Cannot create a Device with an environment that is not allowed
    ...
    And I have already made a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/Product" with body:
      ...
      | name | My Great Product |
    And I note the response field "$.id" as "product_id"
    When I make a "POST" request with "default" headers to "ProductTeamEpr/${ note(product_team_id) }/Product/${ note(product_id) }/foo/Device" with body:
      | path | value     |
      | name | My Device |
    Then I receive a status code "400" with body
      | path             | value                                                                                                                      |
