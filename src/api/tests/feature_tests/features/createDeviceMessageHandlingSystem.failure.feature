Feature: Create MHS Device - failure scenarios
  ...
  Scenario: Cannot create a MHS Device with a Device body that has answered system generated questions for the questionnaire 'spine_mhs'
    ...
    Then I receive a status code "400" with body
      | path             | value                                                                                                                                                                                                                                          |
      | errors.0.code    | VALIDATION_ERROR                                                                                                                                                                                                                               |
      | errors.0.message | Payload contains unexpected fields: {'Address'}. Expected fields are: ['Approver URP', 'Binding', 'DNS Approver', 'MHS FQDN', 'MHS Manufacturer Organisation', 'MHS Service Description', 'Product Name', 'Product Version', 'Requestor URP']. |
    And the response headers contain:
      | name           | value            |
      | Content-Type   | application/json |
      | Content-Length | 295              |
