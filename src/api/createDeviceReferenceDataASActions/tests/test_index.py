...
def mock_product() -> Generator[tuple[ModuleType, CpmProduct], Any, None]:
    ...
    with mock_table(table_name=TABLE_NAME) as client, mock.patch.dict(
        ...
        import api.createDeviceReferenceDataASActions.index as index
...
def test_index_with_questionnaire() -> None:
    ...
    with mock_product() as (index, product):
                ...
                "body": json.dumps(
                    {
                        "questionnaire_responses": {
                            "spine_as_additional_interactions": questionnaire_data
                        }
                    }
                ),
                "pathParameters": {
        ...
        _questionnaire_data = [qr.data for qr in questionnaire_responses]
        assert _questionnaire_data == questionnaire_data
