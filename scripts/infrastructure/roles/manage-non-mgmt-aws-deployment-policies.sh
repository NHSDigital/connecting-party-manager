...
if _validate_current_account "Backups"; then
  policy_name="NHSBackupVaultsPolicy"
  tf_policy=$(_substitute_environment_variables ./scripts/infrastructure/policies/backup-vaults-policy.json)
  aws iam get-policy --policy-arn "arn:aws:iam::${ACCOUNT_ID}:policy/${policy_name}" &> /dev/null
  if [ $? != 0 ]; then
    aws iam create-policy \
      --policy-name "${policy_name}" \
  ...
  policy_name="NHSKmsKeysPolicy"
  tf_policy=$(_substitute_environment_variables ./scripts/infrastructure/policies/kms-keys-policy.json)
  aws iam get-policy --policy-arn "arn:aws:iam::${ACCOUNT_ID}:policy/${policy_name}" &> /dev/null
  if [ $? != 0 ]; then
    aws iam create-policy \
      --policy-name "${policy_name}" \
  ...
  policy_name="NHSBackupDeploymentPolicy"
  tf_policy=$(_substitute_environment_variables ./scripts/infrastructure/policies/backup-deployment-policy.json)
  aws iam get-policy --policy-arn "arn:aws:iam::${ACCOUNT_ID}:policy/${policy_name}" &> /dev/null
  if [ $? != 0 ]; then
    aws iam create-policy \
      --policy-name "${policy_name}" \
