...
function attach_product(){
    ...
    if [ $? -eq 0 ]; then
        ...
        token_response=$(proxygen pytest-nhsd-apim get-token)
        token=$(echo "$token_response" | jq -r '.pytest_nhsd_apim_token')


        app_product=$(curl -X POST \
            https://api.enterprise.apigee.com/v1/organizations/$_org_name/developers/$email_that_owns_app/apps/$app_name/keys/$client_id \
            -H "Authorization: Bearer $token" \
            -H "Content-type:application/json" \
            -d "{\"apiProducts\": [\"$_product_name\"]}")
        echo "$app_product"
        status=$(echo "$app_product" | jq -r '.status')
        echo "$status"

        if [ -z "$status" ] || [ "$status" != "approved" ]; then
          echo "Failed to add product to the app"
          echo "Response: $app_product"
          exit 1
        else
