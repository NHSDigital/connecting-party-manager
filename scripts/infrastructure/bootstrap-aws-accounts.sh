...
function _bootstrap() {
  ...
  local truststore_bucket_name="${PROFILE_PREFIX}--truststore-${version}"
  local state_bucket_name="${PROFILE_PREFIX}--terraform-state-${version}"
  local state_lock_table_name="${PROFILE_PREFIX}--terraform-state-lock-${version}"
