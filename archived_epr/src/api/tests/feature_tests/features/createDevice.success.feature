Feature: Create Device - success scenarios
  ...
  Scenario Outline: Successfully create a Device
    ...
    When I make a "POST" request with "default" headers to "ProductTeamEpr/<product_team_id>/ProductEpr/${ note(product_id) }/dev/Device" with body:
      | path | value     |
      | name | My Device |
  ...
  Scenario Outline: Successfully create a Device with an EPR Product
    ...
    When I make a "POST" request with "default" headers to "ProductTeamEpr/<product_team_id>/ProductEpr/<product_id>/dev/Device" with body:
      | path | value     |
      | name | My Device |
    Then I receive a status code "201" with body
      | path                    | value                      |
