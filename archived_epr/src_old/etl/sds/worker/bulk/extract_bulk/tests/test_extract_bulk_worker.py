...
def mock_s3_client():
    with mock_aws(), mock.patch.dict(
        ...
        extract_bulk.ENVIRONMENT = WorkerEnvironment.build()
        extract_bulk.S3_CLIENT.create_bucket(Bucket=BUCKET_NAME)
        yield extract_bulk.S3_CLIENT
...
def test_extract_worker_pass(
    ...
    put_object(key=WorkerKey.TRANSFORM, body=pkl_dumps_lz4({}))

    # Execute the extract worker
    response = extract_bulk.handler(event={}, context=None)
    assert response == {
        "stage_name": "extract",
        "processed_records": 2,
