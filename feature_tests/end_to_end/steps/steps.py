...
def then_response(context: Context, status_code: str):
    ...
    assert_many(
        assertions=(assert_same_type, assert_same_type, assert_equal),
        expected=(int(status_code), expected_body, expected_body),
        received=(context.response.status_code, response_body, response_body),
    )
