Feature: Create Device - failure scenarios
  ...
  Scenario: Cannot create a Device that already exists
    ...
    Then I receive a status code "500" with body
      ...
      | issue.0.diagnostics              | An error occurred (TransactionCanceledException) when calling the TransactWriteItems operation: Transaction cancelled, please refer cancellation reasons for specific reasons [None, ConditionalCheckFailed] |
